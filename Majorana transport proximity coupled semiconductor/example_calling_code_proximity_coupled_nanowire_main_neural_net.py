...
from majorana_transport_proximity_coupled_nanowire import *
...
N = 50             # Lattice sites
mu_phys = 0.8         # Chemical potential
m = 0.5           # Electron mass
t = 1/(2*m)
b = 1.0          # Zeeman field in x-direction
alpha = 1.0*2       # Rashba Spin-orbit interaction strength
...
for i in range(len(xL_f)):
    ...
    # define loss
    def loss(net_params, inputs, xL_0, xL_f, N,Vmax,sigma,xR,mu_phys,Delta,t,b,alpha,dt):
        ...
        return -Transport_Majorana(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha,dt);

#learning step
    def step(i, opt_state, inputs, xL_0, xL_f, N,Vmax,sigma,xR,mu_phys,Delta,t,b,alpha,dt):
        p = get_params(opt_state)
        g = grad(loss)(p, inputs, xL_0, xL_f, N,Vmax,sigma,xR,mu_phys,Delta,t,b,alpha,dt)
    ...
    Overlap1 = Transport_Majorana(N,Vmax,sigma,xL,xR,mu_phys,delta,t,b,alpha,dt);
    Overlap2 = Transport_Majorana(N,Vmax,sigma,xL2,xR,mu_phys,delta,t,b,alpha,dt);

    # define optimizer
    ##TODO: large scale test with more steps
    opt_init, opt_update, get_params = optimizers.adam(step_size=LR)
    opt_state = opt_init(net_params)

    filename_parameters = 'N100_mu_min055_Delta_05_t_1_T_12_dt_01_Vmax_301_sigma_1_xL0_5_xlF_932_layer_1_neurons_{}__layer_2_neurons_{}_activation1_Relu_activation2_neurons_layer3_{}_sigmoid_alpha1e-3_episodes_200'.format(neurons1,neurons2,neurons3)
    learning_curve = []
    for j in range(episodes):
        then = time.time()
        opt_state = step(i, opt_state, inputs,xL_0, xL_f[i], N,Vmax,sigma,xR,mu_phys,delta,t,b,alpha,dt)
        net_params_tmp = get_params(opt_state)
        xL_tmp = get_xL(net_params_tmp, inputs, xL_0, xL_f[i])
        plt.figure(4)
        plt.plot(xL_tmp)
        Overlap_tmp = Transport_Majorana(N,Vmax,sigma,xL_tmp,xR,mu_phys,delta,t,b,alpha,dt)
        learning_curve.append(Overlap_tmp)
        print(neurons1,1-Overlap_tmp,'The learning episode {} took: '.format(j), time.time()-then, 'seconds')

    net_params = get_params(opt_state)
    xL3 = get_xL(net_params, inputs, xL_0, xL_f[i])
    Overlap3 = Transport_Majorana(N,Vmax,sigma,xL3,xR,mu_phys,delta,t,b,alpha,dt);
    ...
    np.save('Data2/Learning_curve_{}'.format(filename_parameters),learning_curve)
    np.save('Data2/NN_weight_params_{}'.format(filename_parameters),net_params)

    tlist = np.linspace(0,T_total,len(xL3))
    plt.figure(4)
    plt.xlabel('time t/dt')
    plt.ylabel('Left wall position x_L(t)')
    plt.savefig('figures2/Path_search_space_N110_mu_03_Delta_1_t_1_T_5_dt_01_Vmax_301_sigma_1_xL0_5_xlF_10_layer_1_neurons_{}_layer_2_neurons_{}_activation1_Relu_activation2_sigmoid_neurons3_{}'.format(neurons1, neurons2,neurons3))
    ...
    plt.ylabel('Left wall position x_L(t)')
    plt.plot(tlist,xL3)
    plt.savefig('figures2/Final_wall(t)_{}'.format(filename_parameters))
    ...
    #plt.savefig('figures2/Qloss(t)_{}'.format(filename_parameters))
    ...
    plt.savefig('figures2/Learning_curve_{}'.format(filename_parameters))

    print('Total learning took:', time.time()-start_time,'for neurons',neurons2)
    #plt.show()

#np.save('final_fidelity_neurons3layers',final_fidelity_neurons2)
