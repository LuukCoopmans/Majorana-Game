...
from Majorana_transport_socs_np import *
...
N = 100  # Lattice sites
mu_phys = -0.55 #0 #-1.5 #-0.55 # Chemical potential
m = 0.5 # Electron mass
t = 1/(2*m)    # hopping
b = 2.0 #2.0 #4.0 #1.0   # Zeeman field in x-direction
alpha_so = 1.6 #1.6 #2.0 # Rashba Spin-orbit interaction strength
Delta = 0.5 #0.8 #0.7 #0.5 # Superconducting gap

dt = 0.1  # dt=0.01 for accurate
...
if mode=='bang':
    ...
    vL = (xL[1:] - xL[:-1])/dt
elif mode=='bang_load' or 'ramp_load' or 'linear_load':
    date="10-30-2020"
    parent = "/home/path/data"
    path = os.path.join(parent, mode[:-5], "L{}".format(float_formatter(delta_xL))+"+T{}".format(T_total,2), date+"+"+str(num), "data/")
  ...
  assert False, 'mode does not exists'


################ read file test  ################
xL_tmp = xL
start = time.time()
Overlap1 = Transport_Majorana(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha_so,dt)
...
def loss(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha_so,dt):
		return 1-Transport_Majorana(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha_so,dt)


def loss2(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha_so,dt):
		"""penelty on intermediate position that exceeds the two ends
		"""
		if (xL[1:-1] < xL[0]).any() or (xL[1:-1] > xL[-1]).any():
				cost = 1000
		else: 
				cost = 1-Transport_Majorana(N,Vmax,sigma,xL,xR,mu_phys,Delta,t,b,alpha_so,dt)
		return cost
...
def NES_profile_jax(params, params_to_xL, score_function, npop=50, sigma_noise=0.1, alpha=0.05):
  """Natural Evolutionary strategy
...
### cost function setup
score_function = partial(loss, N=N,Vmax=Vmax,sigma=sigma,xR=xR,mu_phys=mu_phys,Delta=Delta,t=t,b=b,alpha_so=alpha_so,dt=dt)
#score_function = partial(loss2, N=N,Vmax=Vmax,sigma=sigma,xR=xR,mu_phys=mu_phys,Delta=Delta,t=t,b=b,alpha_so=alpha_so,dt=dt)
...
params_to_xL = f_vL #f_vL  #f_nn  #f_coarse_position_xL  #f_coarse_velocity_xL #f_position_xL
xL_tmp = params_to_xL(params)
#xL_tmp = xL.copy()
#Overlap1 = Transport_Majorana(N,Vmax,sigma,xL_tmp,xR,mu_phys,Delta,t,b,alpha_so,dt)
...
for i in range(epoch):
  ...
  xL_tmp = params_to_xL(params)
  #Overlap_tmp = Transport_Majorana(N,Vmax,sigma,xL_tmp,xR,mu_phys,Delta,t,b,alpha_so,dt)
...
xL_best = xL_collection[best_index[0][0]]
xL_best2 = x_wall_interpolate(xL_best, T_total, dt/10, mode=1)
xL_collection = np.append(xL_collection, xL_best.reshape(1,-1),axis=0)
Overlap_best = Transport_Majorana(N,Vmax,sigma,xL_best2,xR,mu_phys,Delta,t,b,alpha_so,dt/10)
print('1-Fidelity best:', 1.0-Overlap_best)
...
plt.tight_layout()
f.savefig("./plot/Infidelity.png")
f.savefig("./plot/Infidelity.pdf")
plt.close('all')
