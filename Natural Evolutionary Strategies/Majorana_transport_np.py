...
def Diagonalization(H_BdG,N):
    ...
    return np.real(E), W #W.astype(np.complex128)
...
def Overlap(W_i,W_ev,N):
    """
        ...
        TODO: exist float 64 and complex 128 contradiction in jax 1.16
    """
    U_i = W_i[0:N,0:N];
    V_i  = W_i[N:2*N,0:N];
    ...
    #s1, Overlap1 = np.linalg.slogdet((np.dot(U_i.conj().T,U_ev)+np.dot(V_i.conj().T,V_ev)));
    #s2, Overlap2 = np.linalg.slogdet((np.dot(U_i2.conj().T,U_ev)+np.dot(V_i2.conj().T,V_ev)));
    #print(U_i.dtype)
    #print(V_i.dtype)
    #print(U_i.conj().T.dtype)
    #print(V_i.conj().T.dtype)
    #print(U_i2.dtype)
    #print(V_i2.dtype)
    #print(V_ev.dtype)
    #print(V_ev.dtype)
    #print(np.dot(U_i.conj().T,U_ev).dtype)
    #print(np.linalg.det((np.dot(U_i.conj().T,U_ev)+np.dot(V_i.conj().T,V_ev))).dtype)
    #print(s1, Overlap1)
    #print(s1.dtype)
    #print(Overlap1.dtype)
    #assert False, 'stop'
